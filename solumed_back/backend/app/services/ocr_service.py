...
Flujo:
  ...
  2. Extrae texto con pypdfium2 (digital) o Tesseract+PIL (escaneado)
  3. Parsea las líneas buscando productos por regex
  4. Cruza cada producto con la API del INVIMA (datos.gov.co)

Optimizaciones:
  - Preprocesamiento PIL (escala de grises + sharpen) — sin OpenCV
  - DPI 200 en vez de 300 (2x más rápido para facturas)
  - PSM 6 en Tesseract (bloque de texto uniforme)
  - Páginas procesadas en paralelo con ThreadPoolExecutor
  - Consultas INVIMA en paralelo con asyncio.gather
"""
import re
import asyncio
...
def _preprocesar_imagen(img_pil):
    """Preprocesa imagen PIL — escala de grises + sharpen."""
    from PIL import ImageFilter, ImageOps
    gris = ImageOps.grayscale(img_pil)
    return gris.filter(ImageFilter.SHARPEN)


def _ocr_pagina(img_pil) -> list[str]:
    """Procesa una sola página PIL con Tesseract."""
    import pytesseract
    procesada = _preprocesar_imagen(img_pil)
...
def _extraer_texto_ocr(ruta: str) -> list[str]:
    ...
    if ext == ".pdf":
        ...
        imagenes = [Image.open(ruta).convert("RGB")]

    todas_lineas = []
    if len(imagenes) == 1:
        todas_lineas = _ocr_pagina(imagenes[0])
    else:
        with ThreadPoolExecutor(max_workers=min(len(imagenes), 4)) as executor:
            resultados = list(executor.map(_ocr_pagina, imagenes))

...
export default function DrogeriasPage() {
  ...
  const crearDrog = async () => {
    if (!formDrog.nombre.trim()) { toast("error", "El nombre de la droguería es obligatorio"); return; }
    if (formDrog.nombre.trim().length < 2) { toast("error", "El nombre debe tener al menos 2 caracteres"); return; }
    if (!formDrog.nit.trim()) { toast("error", "El NIT es obligatorio"); return; }
    if (formDrog.nit.trim().length < 5) { toast("error", "El NIT debe tener al menos 5 caracteres"); return; }
    if (formDrog.email && !emailRegex.test(formDrog.email)) { toast("error", "El email de contacto no tiene un formato válido"); return; }
    setGuardando(true);
  ...
  const crearLic = async () => {
    if (!selId) { toast("error", "No se seleccionó ninguna droguería"); return; }
    if (!formLic.vencimiento) { toast("error", "La fecha de vencimiento es obligatoria"); return; }
    if (!formLic.inicio) { toast("error", "La fecha de inicio es obligatoria"); return; }
    if (formLic.vencimiento < formLic.inicio) { toast("error", "La fecha de vencimiento debe ser posterior a la de inicio"); return; }
    const hoyStr = new Date().toISOString().slice(0, 10);
    if (formLic.vencimiento < hoyStr) { toast("error", "La fecha de vencimiento no puede ser anterior a hoy"); return; }
    if (formLic.max_usuarios < 1 || formLic.max_usuarios > 100) { toast("error", "El número máximo de usuarios debe estar entre 1 y 100"); return; }
    setGuardando(true);
  ...
  const crearUser = async () => {
    if (!selId) { toast("error", "No se seleccionó ninguna droguería"); return; }
    if (!formUser.nombre.trim()) { toast("error", "El nombre es obligatorio"); return; }
    if (!formUser.email.trim()) { toast("error", "El correo electrónico es obligatorio"); return; }
    if (!emailRegex.test(formUser.email)) { toast("error", "El correo electrónico no tiene un formato válido"); return; }
    if (!formUser.password) { toast("error", "La contraseña es obligatoria"); return; }
    if (formUser.password.length < 8) { toast("error", "La contraseña debe tener al menos 8 caracteres"); return; }
    setGuardando(true);

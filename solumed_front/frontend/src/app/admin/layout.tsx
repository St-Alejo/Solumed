...
import { FlaskConical, LayoutDashboard, Building2, CreditCard, LogOut, ChevronRight, Menu, X, Sun, Moon, Handshake } from "lucide-react";
import { useTheme } from "@/lib/theme";

// Nav items según rol
const NAV_BASE = [
  { href: "/admin", label: "Dashboard", icon: LayoutDashboard, roles: ["superadmin", "distributor_admin"] },
  { href: "/admin/drogerias", label: "Droguerías", icon: Building2, roles: ["superadmin", "distributor_admin"] },
  { href: "/admin/licencias", label: "Licencias", icon: CreditCard, roles: ["superadmin", "distributor_admin"] },
  { href: "/admin/distribuidores", label: "Distribuidores", icon: Handshake, roles: ["superadmin"] },
];
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && usuario && !(["superadmin", "distributor_admin"].includes(usuario.rol))) router.replace("/recepcion");
  }, [usuario, loading]);

  const rol = usuario?.rol ?? "";
  const NAV = NAV_BASE.filter(n => n.roles.includes(rol));
  const rolLabel = rol === "superadmin" ? "Superadmin" : "Gerente Distribuidor";
        ...
        <aside className={`admin-sidebar${abierto ? " open" : ""}`} style={{
          ...
          <div style={{ padding: "10px 10px 4px" }}>
            <div style={{ padding: "9px 12px", borderRadius: "var(--r-md)", background: "rgba(124,58,237,.1)", border: "1px solid rgba(124,58,237,.2)" }}>
              ...
              <p style={{ color: "#6d28d9", fontSize: 10, marginTop: 1 }}>{rolLabel}</p>
            </div>
          </div>
